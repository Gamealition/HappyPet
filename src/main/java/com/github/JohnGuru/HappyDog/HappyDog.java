...
public final class HappyDog extends JavaPlugin {
    ...
    private void handleTameableHealth(Player p, Entity ent) {
    	...
    	if (T.isTamed())
			if (T.getOwner() == p || p.hasPermission(admin))
	    		handleHealth(p, ent);
	    	else
	    		p.sendMessage("This animal is owned by " + T.getOwner().getName());
    }
    ...
    private void reassign(Player p, Entity e, String newowner, Plugin context) {
    	...
    	if (newowner.length() > 0) {
    		...
    		if ( owner != null && (owner.hasPlayedBefore() || owner.isOnline())) {
    			...
    			p.sendMessage("Invalid owner: " + newowner);
    	} else {
    		/*
    		...
    		else if (animal.isTamed())
    			if (animal.getOwner() == p) {
            		animal.setOwner(null);
            		p.sendMessage("Animal has been released.");
    			}
    			else
    				p.sendMessage("This animal is owned by " + animal.getOwner().getName());
    		else
    			p.sendMessage("This animal is not tamed.");
    	}
